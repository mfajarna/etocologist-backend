        ...
        @endif
         <div class="card">
             <div class="card-header">
                Ubah Obat <?= $item->nama_obat ?>
             </div>
         <div class="card-body">
             <form action="{{ route('obat.update', $item->id) }}" class="w-full" method="POST">
                @csrf
                @method('PUT')
                <div class="form-group">
                    <label for="nama_obat">Nama Obat</label>
                    <input type="type" class="form-control" id="nama_obat" name="nama_obat" aria-describedby="nama_obat" placeholder="Masukan Nama Obat" value="{{ old('nama_obat') ?? $item->nama_obat }}">
                ...
                <div class="mt-2">
                    <button class="btn btn-primary" type="submit">Simpan</button>
                     <a href="{{ route('obat.index') }}" class="btn btn-outline-secondary" role="button">Kembali</a>
                </div>
            </form>
         </div>
        </div>
